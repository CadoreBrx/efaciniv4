...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    cb(null, `${Date.now()}${ext}`); // Nome do arquivo: timestamp + extensão
  }
});

// Opções adicionais, se necessário (ex: limite de tamanho, tipo de arquivo permitido)
const upload = multer({ storage });

// Função para lidar com o upload de imagem
export const uploadImage = async (req: Request, res: Response): Promise<Response> => {
  try {
    // Verifica se há um arquivo enviado via upload.single('image')
    if (!req.file) {
      return res.status(400).json({ error: "No file uploaded" });
    }

    // Aqui você pode acessar o caminho do arquivo salvo em req.file.path
    const imagePath = req.file.path;

    // Lógica adicional: salve o caminho da imagem no banco de dados ou faça o que for necessário
...
// Métodos existentes do controlador (index, store, update, etc.)
...
export const saveMessage = async (
  ...
  const { companyId } = req.user;
  const { message, mediaPath, mediaName } = req.body;
  const { id } = req.params;
